...
import { updateReservationStatusSchema } from "../validations/reservationValidation.js";
export const createReservation = async (req, res) => {
    try {
        const { branchId, guests, reservationTime, notes, items } = req.body;
        const user = req.user;
        if (!user) {
            res.status(401).json({
        ...
        if (reservationHour <
            ...
            reservationHour >=
                branch.closingHour) {
            res.status(400).json({
                message: "Branch is closed at selected reservation time"
            });
            return;
        }
        ...
        const reservationsAtSameTime = await prisma.reservation.findMany({
            where: {
        ...
        const totalGuests = reservationsAtSameTime.reduce((sum, reservation) => sum + reservation.guests, 0);
        if (totalGuests + guests >
            branch.capacity) {
            res.status(400).json({
                message: "Branch capacity exceeded for this time slot"
            });
            return;
        }
        ...
        res.status(201).json({
            message: "Reservation and pre-order created successfully",
...
export const getBranchReservations = async (req, res) => {
    try {
        ...
        if (user.role === "BRANCH_MANAGER" &&
            ...
            res.status(403).json({
                message: "Branch managers can only access their own branch reservations"
            });
            return;
        }
        const reservations = await prisma.reservation.findMany({
...
export const updateReservationStatus = async (req, res) => {
    try {
        ...
        if (!chef?.branchId) {
            res.status(400).json({
                message: "Chef is not assigned to a branch"
            });
            return;
        }
        ...
        if (reservation.branchId !==
            chef.branchId) {
            res.status(403).json({
                message: "You can only update reservations for your branch"
            });
            return;
        }
...
export const cancelReservation = async (req, res) => {
    try {
        ...
        if (String(reservation.customerId) !==
            String(user.id)) {
            res.status(403).json({
