...
export const getAnalytics = async (_req, res) => {
    try {
        ...
        const preparingReservations = await prisma.reservation.count({
            where: {
                status: "PREPARING"
            }
        });
        const readyReservations = await prisma.reservation.count({
            where: {
                status: "READY"
            }
        });
...
export const getBranchAnalytics = async (req, res) => {
    try {
        ...
        const totalRevenue = branchOrders.reduce((sum, order) => sum + order.totalPrice, 0);
        res.status(200).json({
            totalCustomers,
            totalChefs,
            totalBranches: 1,
