...
export const createBranch = async (req, res) => {
    try {
        const { name, location, phone, capacity, openingHour, closingHour } = req.body;
        const branch = await prisma.branch.create({
            data: {
                ...
                openingHour: Number(openingHour) || 10,
                closingHour: Number(closingHour) || 23
            }
        });
...
export const updateBranch = async (req, res) => {
    try {
        const id = req.params.id;
        const { name, location, phone, revenue, capacity, openingHour, closingHour } = req.body;
...
export const assignManager = async (req, res) => {
    try {
        ...
        if (manager.role !==
            "BRANCH_MANAGER") {
            res.status(400).json({
                message: "User is not a branch manager"
            });
            return;
        }
